# FilePulse 简介，{{VERSION}} 正式版发布，不需要转义，正式原生支持Windows、Mac、Linux同时支持x86_64和aarch64双架构，不同操作系统的版本功能完全一样。由于加壳，杀毒可能误报，请杀毒软件添加信任，媒体测试可以申请无壳版本。

FilePulse 是一款高效的对象信息搜索工具，对象包括 **文件** 和 **文件夹**，支持 **中文搜索**、**拼音搜索**、**拼音首字母搜索**、**文件内容搜索** 与 **集群搜索**。  
产品提供完整工作流：**文件名搜索 + 文件内容搜索 + 快速启动器 + 文件去重 + 远程预览与传输 + 文件夹自动远程同步 + 快速预览 + 共享访问**。  
文件夹大小可 **实时计算**，文件变动会 **自动同步** 到本系统，系统支持 **HTTP1.1/HTTP2/HTTP3** 协议。
...
---
## 核心功能
- 核心功能1：文件名搜索（类似 Everything），支持复杂筛选，毫秒级响应。
- 核心功能2：文件内容搜索（类似 AnyTXT），支持提前按文件名条件筛选索引范围，检索响应快。
- 核心功能3：快速启动器（类似 Listary），按习惯排序结果，快速定位并启动。
- 核心功能4：文件去重，按大小与特征快速比对重复文件，支持安全复核与清理。
- 核心功能5：远程预览与传输，支持跨节点预览、上传、下载与远程定位。
- 核心功能6：文件夹自动远程同步，支持本地/远程目录映射、周期同步、稳定重连与删除记录追踪。
- 核心功能7：共享访问，支持受控共享入口，便于协作分发。
- 核心功能8：启动器面板（类似音速启动），可整理常用工具并一键启动。
- 扩展能力：集群组网，最多支持 256 台服务器，节点平等，支持跨节点搜索与远程操作。

---

## 全链路工作流（v{{VERSION}}）

1. 文件名搜索：按名称、路径、大小、扩展名快速筛选。
2. 文件内容搜索：支持全文检索、评分排序与命中高亮。
3. 快速启动器：按关键字快速启动应用与目录。
4. 文件去重：定位冗余文件并进行安全清理。
5. 远程传输：跨节点上传、下载、定位文件。
6. 文件夹自动远程同步：本地与远程映射目录持续自动同步，更新更省心。
7. 快速预览：轻量预览文档、图片与多媒体内容。
8. 共享访问：按需生成共享入口，便于协作交付。
...
## 内存占用对比
FilePulse对全盘227万文件名索引，对全盘71120个python文件内容进行索引，这个是对标anytxt文件内容搜索功能。FilePulse内存稳定后最终占用264MB，硬盘占用877MB。anytxt内存占用307MB，磁盘占用1.4GB。

如果关掉文件内容索引功能只进行文件名搜索，这个对标Everything，因为everything对文件内容搜索支持只在初级阶段。内存稳定后，此时FilePulse占253MB，everything是采用双进程模式总共占用246MB内存，我支持的功能多，多占几MB内存也正常吧。

## 与everything的比较文件名搜索，时间统计方式为浏览器请求查询接口，服务器等待响应时间，全部测速都非命中缓存

用户的版本可能是加壳的，会导致性能下降，媒体性能测试可以申请无壳版本，未来将全面提供无壳版本。

| 测试项目                            | FilePulse {{VERSION}} (自带统计文件夹大小和扩展名索引)                                                       | Everything 1.4.1.1026+统计文件夹大小+扩展名索引 | Everything1.5.0.1391a+统计文件夹大小+扩展名索引        |
|---------------------------------|-----------------------------------------------------------------------------------------------|-------------------------------------|--------------------------------------------|
| 200万文件扫描全盘速度+构建存储结构速度           | <span style="color: red;">2.07秒</span>                                                        | 5.73秒                               | 4.26秒                                      |
...
| 测试项目                          | FilePulse {{VERSION}}                                                                                    | Anytxt 1.3.2477                   |
|-------------------------------|----------------------------------------------------------------------------------------------------|-----------------------------------|
